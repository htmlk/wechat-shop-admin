...
module.exports = class extends Base {
  async indexAction() {
    ...
    const hotGoods = await this.model('goods').field(['id', 'name', 'list_pic_url', 'retail_price', 'goods_brief']).where({is_hot: 1,is_on_sale: 1}).limit(10).select();
    ...
    //   const childCategoryIds = await this.model('category').where({parent_id: categoryItem.id}).getField('id', 5);
