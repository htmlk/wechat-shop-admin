...
module.exports = class extends Base {
  ...
  async submitAction() {
    ...
    //判断是直接配送还是物流配送
    const switchAddressflag = this.post('switchAddressflag');
    const roomNo= this.post('roomNo');
    const roomName= this.post('roomName');
    const roomPhone= this.post('roomPhone');
    const hotelName=this.post('hotelName');
    const hoteltime=this.post('hoteltime');
    var checkedAddress={}
    if (switchAddressflag==0) {
      //直接配送
       checkedAddress={
        address:hotelName+roomNo,
        wl_time:hoteltime,
        name:roomName,
        mobile:roomPhone
      }
    }else if(switchAddressflag==1){
      //物流配送
        checkedAddress = await this.model('address').where({ id: addressId }).find();
    ...
    const orderInfo = {
      ...
      //自提时间
      wl_time:checkedAddress.wl_time,
